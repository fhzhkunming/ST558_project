...
```
# Vignette of FoodData Central API
...
Teleport API does not require a key for access, making it less complex to use compared to other APIs that require a key.  

#### Create a function to retrieve median salaries of 50 countries worldwide.  
...
#### Creat a function to query salaries of 52 first tier cities worldwide
```{r}
...
#### Visulization of 52 cities median salaries
...
#### Creat a function to query US state capital cities available in the API
```{r}
...
#### Compare life quality scores for all US cities that available in the Teleport API
```{r}
...
Then, I used code to find out top 10 median salary jobs across 52 first tier cities
```{r}
# Load the required libraries
library(dplyr)
